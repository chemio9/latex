...
\begin{figure}
  ...
  \caption{$y=x^2$}\label{fig:square}
\end{figure}
...
当$n \to +\infty$时，
这$n$个矩形的面积和可近似认为是 $f(x) = y = x^{2},x\in [0, 1] $ 与 X 轴围成的面积。
而
\[
...
\begin{figure}[htbp]
  ...
  \caption{a 到 b 的矩形逼近}\label{fig:absim}
\end{figure}
...
\subsubsection{正弦式交变电流有效值的推导}
我们都知道 $I_m = \frac {I_0}{\sqrt 2}$, $U_m=\frac{U_0} {\sqrt 2}$, 那么怎么推导呢？
...
则在极短时间$\dif t$内，做的功
\[
  \dif W = \frac{U^2(t)}{R} \cdot \dif t
\]
而一个周期内做的功可看作是无数个极短时间内做的功之和。
故有
\begin{align*}
  W = \frac {U_m^2}{R} \cdot \frac T 2 &=
  \int_0^{\frac T 2}
  \frac{U^2(t)}{R} \cdot \dif t  \\
  &= \frac{U_0^2}{R} \int_0^{\frac T 2}\sin^2 (\omega t + \varphi) \dif t \\
\end{align*}

而对于这个积分，令$x=\omega t + \varphi$, 则原积分
\begin{align*}
  I &= \int \sin^2 (\omega t + \varphi) \dif t \\
  &= \int \sin^2 x \dif (\frac x w - \frac \varphi w) \\
  &= \frac 1 \omega \int \sin^2 x \dif x \\
  &= \frac 1 \omega (\frac 1 2 x - \frac 1 4 \sin 2x) \\
  \intertext{代入$x=\omega t+\varphi$得到}
  &= \frac 1 \omega \left[ \frac 1 2 (\omega t + \varphi ) - \frac 1
  4 \sin 2(\omega t + \varphi )\right]
\end{align*}

记这个函数为$F(t)$, 那么
\begin{align*}
  \frac {U_m^2}{R} \cdot \frac T 2 &=
  \int_0^{\frac T 2} \sin^2 (\omega t + \varphi) \dif t \cdot
  \frac{U_0^2} {R} \\
  &= \frac{U_0^2}{R} \left[ F (\frac T 2 ) - F (0) \right]  \\
  &= \frac T 4 \cdot \frac {U_0^2} {R}
\end{align*}
